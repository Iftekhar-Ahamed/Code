...
void solve(){
    ...
    cin>>s;
    doubleHashing ob;
    vector<vector<ll>>hash(s.size()+1,vector<ll>(2));
    ob.calculateHashValue(s,hash);

    for(ll i=1,j=s.size();i<s.size();i++,j--){
        
        ll suffixHash1 = ((hash[s.size()][0] - hash[j-1][0])*ob.invType1[j-1])%ob.mod1,suffixHash2 = ((hash[s.size()][1] - hash[j-1][1])*ob.invType2[j-1])%ob.mod2;
        if(suffixHash1<0){
            suffixHash1+=ob.mod1;
        }
        if(suffixHash2<0){
        ...
        if(hash[i][0]==suffixHash1 && hash[i][1]==suffixHash2){
            cout<<i<<" ";
        }
    }
    cout<<nn;
