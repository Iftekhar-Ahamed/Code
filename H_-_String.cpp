...
struct doubleHashing
{
    ...
    bool ok(vector<ll> &hashMain, ll ps)
    {
        ll n = hashMain.size() - 1;

        ll hashOfPatern1 = hashMain[ps], hashOfPatern2 = hashMain[ps];
        ll count = 0;
        ll i;
        for (i = 0; i + ps <= n; i += ps)
        {
            ll th1 = hashMain[i + ps] - hashMain[i];
        ...
        if (i < n)
        {
            ...
            ll th1 = hashMain[n] - hashMain[i];

            th1 = (th1 * invType1[i]) % mod1;

            if (th1 < 0)
                th1 += mod1;
            if (th1 == hashMain[need])
            {
                return true;
            }
            else
